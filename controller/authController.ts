...
export const updateOneUser = async(req:Request, res: Response): Promise<Response>=>{
    try {
        const {id} = req.params;
        const user = await authModel.findByIdAndDelete(
            id
        )
...
export const createAccount = async(req:Request, res: Response): Promise<Response>=>{
    try {
        ...
        const user = await authModel.create({userName, avatar, email,password: hash})
        return res.status(200).json({message: "Get all users", data: user})
    } catch (error:any) {
        return res.status(404).json({message: error.message})
    }
...
export const signInAccount = async(req:Request, res: Response): Promise<Response>=>{
    try {
        ...
        if (user) {
            ...
            if(passed){
                return res.status(200).json({message: `Welcom Back ${user?.userName}`, data: user})
            }
        }else{
